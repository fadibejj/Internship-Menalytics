...
#        Take the candidates part of the table and the Lists part of the table as seperate DFs
        ...
        sumCandidatesVotesDF=self.candidatesVotesDF.groupby(['List']).sum().reset_index() #Total number of votes per list (sorted (lists 1,2,3)) in the candidates votes DF # reset_index to get normal index 0,1,2
        sumListExtraVotesDF = self.listExtraVotesDF.groupby(['ListName']).sum().reset_index() #Total number of extra votes per list, note: here votes are already sorted by lists but needed in case lists are not sorted in order
        ...
        totalVotesDF = totalVotesDF[~(totalVotesDF['Total Votes'] < self.hasel)]     # ~ removes the row (list) less than hasel
        while totalVotesDF.shape != tempTotalVotesDF.shape:     #while the shape of the tempDF doesnt match the shape of the original DF, another list must be removed
            self.hasel = totalVotesDF['Total Votes'].sum() / totalNumberOfSeats
            tempTotalVotesDF=totalVotesDF
            totalVotesDF = totalVotesDF[~(totalVotesDF['Total Votes'] < self.hasel)]
        ...
        self.listExtraVotesDF = self.listExtraVotesDF[self.listExtraVotesDF['ListName'].isin(passed7aselList)].reset_index(drop=True) #remove the lists that did not pass the hasel from the listExtraVotesDF
        ...
        self.seatsRepartitionPerList = pd.concat([totalVotesDF['ListName'], seatsDivisionRounded, seatsDivisionDecimal], axis=1, keys=['ListName', 'Total Seats', 'Decimal'])    #dataframe of listname, current number of seats per list, decimal part of calculation
        while totalNumberOfSeats != sum(self.seatsRepartitionPerList['Total Seats']):    #while the totalNumberOfSeats originally calculated != sum of seats given for each list, one seat should be added to the max decimal list
            maxDecimal = max(self.seatsRepartitionPerList['Decimal'])    #maximum of the decimals
            self.seatsRepartitionPerList.loc[self.seatsRepartitionPerList['Decimal']==maxDecimal,'Total Seats']+=1    #one seat is added
            self.seatsRepartitionPerList.loc[self.seatsRepartitionPerList['Decimal']==maxDecimal,'Decimal']=0 #set the maxDecimal to 0 so that it is not used in a next iteration as maxDecimal
        self.seatsRepartitionPerList = self.seatsRepartitionPerList.drop('Decimal', 1)  #drop the decimal column after finishing calculcations # drop the Decimal column on the 1 (or column) axis  
        ...
        votesDF = pd.concat([self.votesResultsDF['Da2ira'],self.listExtraVotesDF['ListName'],self.listExtraVotesDF['ListExtraVote'], votesDF], axis=1)    #output DF form of the election results
        votesDF.to_csv(self.electionResultsFile, index=False, encoding='utf-8-sig')    #write DF to a csv file

Saida1=electionResultsGen('Kes-Jbeiltest.csv')
Saida1.getResults()